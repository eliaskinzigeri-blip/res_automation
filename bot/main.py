...
def run_script(name):
    ...
    result = subprocess.run(["python", f"bot/{name}.py"], capture_output=True, text=True)
...
def main():
    ...
    if os.path.exists("final_article.md"):
        ...
        os.makedirs("src/content/guides", exist_ok=True)
        ...
        with open(f"src/content/guides/{filename}", "w", encoding="utf-8") as f:
